package buildersRiskTestsTexas;                                                                     // THIS TEST RUNS FOR 2 MINUTES AND 41 SECONDS
...
public class BuildersRiskCoverageSummaryTest extends ReusablePolicyMaker{
	...
	public void CoverageSummaryTest2() throws InterruptedException{
		...
		if(insuredNameFound.contains(TXData[17].toUpperCase())) {
			System.out.print("\nBusiness Name is correctly displayed");
			if(!TXData[66].equals("")) {
				if(insuredNameFound.contains(TXData[66].toUpperCase())) {
			...
			if(!TXData[67].equals("")) {
				if(insuredNameFound.contains(TXData[67].toUpperCase())) {
			...
			if(!TXData[68].equals("")) {
				if(insuredNameFound.contains(TXData[68].toUpperCase())) {
		...
		if(insuredaddress[0].contains(TXData[20].toUpperCase()) && insuredaddress[0].contains(TXData[22].toUpperCase())) {
			if(insuredaddress[0].contains(TXData[23].toUpperCase()) && insuredaddress[0].contains(TXData[24].toUpperCase())) {
		...
		if(insuredaddress[1].contains(TXData[9])) {
			System.out.print("\nInsured phone number is correctly displayed");
		}
		else {
			Assert.fail("INSURED'S PHONE NUMBER MISMATCH");
		}
		if(insuredaddress[2].contains(TXData[10].toUpperCase())) {
		...
		if(driver.findElement(By.xpath(loc.getProperty("buildersfinalpolicytype"))).getText().contains(TXData[0].toUpperCase())) {
			if(driver.findElement(By.xpath(loc.getProperty("buildersfinalpolicytype"))).getText().contains(TXData[1])) {
				System.out.print("\nPolicy Type is successfully displayed");
			}
			else {
				Assert.fail("POLICY TYPE MISMATCH - EXPECTED: " + TXData[1]);
			}
		}
		else {
			Assert.fail("POLICY TYPE STATE MISMATCH - EXPECTED: " + TXData[0]);
		}
		...
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicyselectedagent"))).getText().contains(TXData[6])) {
	...
	public void CoverageSummaryTest3() throws InterruptedException{
		...
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicyrisklocation"))).getText().contains(TXData[2].toUpperCase())){
			if(driver.findElement(By.xpath(loc.getProperty("builderspolicyrisklocation"))).getText().contains(TXData[4].toUpperCase())){
				if(driver.findElement(By.xpath(loc.getProperty("builderspolicyrisklocation"))).getText().contains(TXData[5].toUpperCase())){
					checker = true;
		...
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicyprojecttype"))).getText().contains(TXData[7])) {
			System.out.print("\nProject Type is correctly displayed");
		}
		else {
			Assert.fail("PROJECT TYPE MISMATCH");
		}
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicycounty"))).getText().contains(TXData[27].toUpperCase())) {
			System.out.print("\nRisk County is correctly displayed");
		}
		else {
			Assert.fail("RISK COUNTY MISMATCH");
		}
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicywindpool"))).getText().contains(TXData[30].toUpperCase())) {
			System.out.print("\nWindpool value is correctly displayed");
		}
		else {
			Assert.fail("WINDPOOL VALUE MISMATCH");
		}
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicyprotectionclass"))).getText().contains(TXData[34].toUpperCase())) {
			System.out.print("\nProtection Class is correctly displayed");
		}
		else {
			Assert.fail("PROTECTION CLASS MISMATCH");
		}
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicylosshistoryTX"))).getText().contains("0")) {
			System.out.print("\nLoss History is correctly displayed");
		}
		else {
			Assert.fail("LOSS HISTORY MISMATCH");
		}
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicysecurityscore"))).getText().contains(TXData[81].toUpperCase())) {
			System.out.print("\nSecurity Score is correctly displayed");
		}
		else {
			Assert.fail("SECURITY SCORE MISMATCH");
		}
	...
	public void CoverageSummaryTest4() throws InterruptedException{
		...
		Double completedvalue = Double.parseDouble(TXData[37].toUpperCase());
		...
		if(collapse.contains("Form CTLBR 009") && collapse.contains("INCLUDED")){
		...
		if(TXData[41].equals("") || TXData[41].contains("$10,000")) {
			...
			String propertytransit = TXData[41].substring(TXData[41].length()-6);
		...
		if(TXData[42].equals("") || TXData[42].contains("$5,000")) {
			...
			if(softcost.contains(TXData[42]) && softcost.contains("\t$")){
				System.out.print("\nSoft Cost Coverage limit is correctly displayed");
			}
			else {
				Assert.fail("SOFT COST COVERAGE LIMT IS MISSING/MISMATCH. EXPECTED: " + TXData[42]);
			}
		}
		...
		String deductible = TXData[38].substring(0, TXData[38].length()-3) + "," + TXData[38].substring(TXData[38].length()-3);
		...
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicywindhail"))).getText().contains(TXData[39])) {
	...
	public void CoverageSummaryTest5() throws InterruptedException{
		...
		if(driver.findElement(By.xpath(loc.getProperty("builderspolicytotalcovfees"))).getAttribute("innerText").contains("TOTAL POLICY PREMIUM, ASSESSMENTS, FEES AND ALL SURCHARGES\t$")) {
			if(driver.findElement(By.xpath(loc.getProperty("builderspolicytotalcovfees"))).getAttribute("innerText").contains(totalprem[1])) {
				System.out.print("\nTOTAL POLICY PREMIUM, ASSESSMENTS, FEES AND ALL SURCHARGES is correctly displayed");
			}
			else {
				Assert.fail("THERE SHOULD NOT BE ANY FEES FOR TEXAS (PREMIUM AND FEES SECTION)");
			}
		}
		else {
