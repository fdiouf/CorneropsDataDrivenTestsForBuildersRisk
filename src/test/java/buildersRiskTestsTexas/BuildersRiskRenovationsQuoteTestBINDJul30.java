package buildersRiskTestsTexas;                                                                     // THIS TEST RUNS FOR 2 MINUTES AND 23 SECONDS
...
public class BuildersRiskRenovationsQuoteTestBINDJul30 extends BuildersRiskBaseClass {
	...
	@DataProvider(name = "RenovationsData")
	public String[][] getData() throws IOException {
		// Get data from Excel
		String path = ".\\src\\main\\resources\\data\\BuilderRiskData.xlsx";
		XLUtility xlutil = new XLUtility(path);

		int totalrows = xlutil.getRowCount("TXRenovationsPolicyDataBINDJ30");
		int totalcols = xlutil.getCellCount("TXRenovationsPolicyDataBINDJ30", 1);

		String loginData[][] = new String[totalrows][totalcols];

		for (int i = 1; i <= totalrows; i++) {
			for (int j = 0; j < totalcols; j++) {
				loginData[i - 1][j] = xlutil.getCellData("TXRenovationsPolicyDataBINDJ30", i, j);
			}
		}
		return loginData;
	}

	@Test(dataProvider = "RenovationsData", description = "Builders Risk Start Quote Walkthrough", enabled = true, priority = 0)
	public void buildersRiskNewQuote(String newquotestate, String newquotepolicytype, String newquoteaddress, String newquoteaddress2, 
			...
			String buildersmailingcity, String buildersmailingstate, String buildersmailingzip, String buildersmailingzip4, String builderslocationzip4, 
			String builderscounty, String buildershailscore, String builderswindpool, String builderstier, String buildersprotectionclass, String buildersfirestation, 
			String buildersfirehydrant, String builderscoveragerequired, String buildersstructurevalue, String buildersworkvalue, String buildersdeductible, 
			String builderswindhaildeductible, String buildersconstructioncostlimit, String builderspropertytransitlimit, String builderssoftcostlimit, 
			String buildersstructuretype, String buildersconstructionyear, String builderselectricalyear, String buildersroofingyear, 
			String buildersplumbingyear, String buildershvacyear, String builderssquarefootage, String buildersroofmaterial, String buildersconstructiontype, 
			String builderspriorinscarrier, String buildersnumberofstories, String builderspriorinsenddate, String builderspriorinspremium, String buildersinteresttype, 
			String buildersinterestname, String buildersinterestaddress, String buildersinterestaddress2, String buildersinterestcity, 
			String buildersintereststate, String buildersinterestzip, String buildersinterestzip4, String buildersloannumber, String builderstinsuredtype, 
			...
			String builderstinsuredaddress2, String builderstinsuredcity, String builderstinsuredstate, String builderstinsuredzip, String builderslegalname, 
			String buildersconstructionstarted, String buildersconstructionstartdate, String builderspercentcomplete, String builderscrimescore, 
			String builderscrimetext, String builderssecuritydesc, String builderssecuritypoints, String buildersjudgementrate, String buildersjudgementreason)
		...
		if(!newquotestate.equalsIgnoreCase("Texas")) {
			Assert.fail("RAN FLORIDA TEST WITH FLORIDA ADDRESS");
			driver.quit();
		}
		...
		driver.findElement(By.id(loc.getProperty("searchagentstage"))).click();
		...
		driver.findElement(By.xpath(loc.getProperty("buildersadditionaldetails"))).sendKeys("TEST");
...
//		buildercounty.selectByVisibleText(builderscounty);
		...
		driver.findElement(By.id(loc.getProperty("builderswindpool"))).click();
		driver.findElement(By.id(loc.getProperty("builderswindpool"))).sendKeys("Out");
		...
		if(!buildersfirehydrant.equals("")) {
			...
			if(driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).getAttribute("value").equals("")) {
				driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).click();
				driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).sendKeys("L");
			}
		}
		...
		Thread.sleep(500);
		for(int i=0; i<2; i++) {
			driver.findElement(By.id(loc.getProperty("builderscoveragerequired"))).click();
			driver.findElement(By.id(loc.getProperty("builderscoveragerequired"))).sendKeys(builderscoveragerequired);
		}
		if(builderscoveragerequired.equalsIgnoreCase("Yes")){
			driver.findElement(By.id(loc.getProperty("buildersstructurevalue"))).click();
			driver.findElement(By.id(loc.getProperty("buildersstructurevalue"))).sendKeys(buildersstructurevalue);
		}
		driver.findElement(By.id(loc.getProperty("buildersworkvalue"))).click();
		driver.findElement(By.id(loc.getProperty("buildersworkvalue"))).sendKeys(buildersworkvalue);
		...
		if (!buildersprojecttype.equalsIgnoreCase("New Construction")) {
			if(!buildersconstructionyear.equals("")) {
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).click();
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).clear();
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).sendKeys(buildersconstructionyear);
			}
			else {
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).click();
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).clear();
				driver.findElement(By.id(loc.getProperty("buildersconstructionyear"))).sendKeys("1974");
			}
...
//		driver.findElement(By.id(loc.getProperty("buildersexperiencedyes"))).click();
		...
		driver.findElement(By.id(loc.getProperty("buildersvacantbuildingyes"))).click();
		driver.findElement(By.id(loc.getProperty("buildersforeclosureyes"))).click();
		driver.findElement(By.id(loc.getProperty("buildersforeclosuremonths"))).click();
		driver.findElement(By.id(loc.getProperty("buildersforeclosureno"))).click();
...
//		driver.findElement(By.xpath(loc.getProperty("builderssecuritydesc"))).sendKeys(builderssecuritydesc);
		String securitypoints= driver.findElement(By.xpath("//*[@id=\"CalculatedResult-SecurityScore_1\"]")).getText();
		
		if(builderssecuritypoints.equals("")){
			
			//*[@id="CalculatedResult-SecurityScore_1"]   //*[@id="AddSecurityPointsDesc_1"]
			
			driver.findElement(By.xpath("//*[@id=\"AddSecurityPointsDesc_1\"]")).click();
			
			driver.findElement(By.xpath("//*[@id=\"AddSecurityPointsDesc_1\"]")).sendKeys(securitypoints);
		}
		...
		if(!buildersprojecttype.equalsIgnoreCase("Structural Renovation")) {
			System.out.print("Policy Number: " + driver.findElement(By.xpath(loc.getProperty("builderspolicynumber"))).getText());
		}
		else {
			try {
				driver.findElement(By.xpath(loc.getProperty("builderspolicynumber"))).getText();
			} catch (Exception e) {
				System.out.print("\nCan NOT bind Structural Renovation quote into policy");
			}
		}
		Action.fluentWait(driver, null, 0);
	}
