package buildersRiskTestsTexas;                                                                     // THIS TEST RUNS FOR 1 MINUTES AND 50 SECONDS
...
public class BuildersRiskRenovationsQuoteTestNoBind extends BuildersRiskBaseClass {
	...
	public String[][] getData() throws IOException {
		...
		int totalrows = xlutil.getRowCount("TXRenovationsPolicyDataNB");
		int totalcols = xlutil.getCellCount("TXRenovationsPolicyDataNB", 1);

		String loginData[][] = new String[totalrows][totalcols];

		for (int i = 1; i <= totalrows; i++) {
			for (int j = 0; j < totalcols; j++) {
				loginData[i - 1][j] = xlutil.getCellData("TXRenovationsPolicyDataNB", i, j);
			}
		}
		return loginData;
	}
