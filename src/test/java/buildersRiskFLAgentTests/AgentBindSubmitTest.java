package buildersRiskFLAgentTests;														// THIS TEST RUNS FOR 2 MINUTES AND 50 SECONDS
...
public class AgentBindSubmitTest extends BuildersRiskBaseClass {
	...
	public void buildersRiskNewQuote(String newquotestate, String newquotepolicytype, String newquoteaddress,
		...
		boolean h = true;
		if(driver.findElement(By.id(loc.getProperty("builderslocationaddressagent"))).getAttribute("disabled").equals("true")) {
			h = false;
...
//		driver.findElement(By.xpath(loc.getProperty("buildersterrorism"))).click();
	...
	public void buildersRiskBindSubmit3() throws InterruptedException{
		...
		if(check == false) {
			Assert.fail("UNVERIFIED PAYMENTS SHOULD BE HIDDEN FOR AGENT");
		}
		try {
	...
	public void buildersRiskBindSubmit8() throws InterruptedException {
		...
		try {
			...
			em = false;
			System.out.print("\nEmail Quote is hidden for agents as expected");
		}
		if(em==true) {
			Assert.fail("EXPECTED EMAIL QUOTE TO BE HIDDEN FOR AGENTS");
		}
		Boolean check = false;
		try {
...
//
//		driver.findElement(By.xpath(loc.getProperty("buildersbindsubmitbtn"))).click();
//		System.out.print("Created Policy: " + driver.findElement(By.xpath(loc.getProperty("builderspolicynumber"))).getText());
