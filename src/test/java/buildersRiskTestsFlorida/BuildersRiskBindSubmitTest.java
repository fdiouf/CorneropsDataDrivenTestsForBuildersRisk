package buildersRiskTestsFlorida;														// THIS TEST RUNS FOR 2 MINUTES AND 27 SECONDS
...
public class BuildersRiskBindSubmitTest extends BuildersRiskBaseClass {
	...
	public void buildersRiskNewQuote(String newquotestate, String newquotepolicytype, String newquoteaddress,
		...
		if(buildersfirehydrant.equalsIgnoreCase("Less than 1000 feet")) {
			driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).sendKeys("L");
		}
		else {
			driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).sendKeys("O");
		}
		driver.findElement(By.id(loc.getProperty("buildersfirehydrant"))).click();
...
//		driver.findElement(By.id(loc.getProperty("buildersexperiencedyes"))).click();
	...
	public void buildersRiskBindSubmit1(String newquotestate, String newquotepolicytype,
	...
	@Test(description = "T18818	TC03 - Builder Risk - Bind Submit Application - Premiums and Fees", enabled = true, priority = 2)
	public void buildersRiskBindSubmit2() throws InterruptedException, ParseException {
		...
		if(fees == true) {
			String flemergency = "";
			if(driver.findElement(By.xpath(loc.getProperty("buildersbindflfire"))).getAttribute("innerText").contains("$") && driver.findElement(By.xpath(loc.getProperty("buildersbindflfire"))).getAttribute("innerText").contains(".")) {
				System.out.print("\nFL Emergency Trust Fund is displaying in currency format as expected");
				flemergency = driver.findElement(By.xpath(loc.getProperty("buildersbindflfire"))).getAttribute("innerText").substring(1).replace(",", "").replace("\n", "");
			}
			else {
				Assert.fail("FL EMERGENCY TRUST FUND OR FL FIRE MARSHALL FEE IS NOT DISPLAYING IN CURRENCY FORMAT");
			}
			String[] flfirefee = flemergency.split("[$]");
			String flFIGA2023A = "";
			Date date1 = new SimpleDateFormat("MM/dd/yyyy").parse(effective);
			Date date2 = new SimpleDateFormat("MM/dd/yyyy").parse("09/30/2025");
			if(date1.before(date2)) {
				System.out.print("\nFIGA 2023 A SHOULD be displayed");
				if(driver.findElement(By.xpath(loc.getProperty("buildersbindFIGA2023A"))).getAttribute("innerText").contains("$") && driver.findElement(By.xpath(loc.getProperty("buildersbindFIGA2023A"))).getAttribute("innerText").contains(".")) {
					System.out.print("\nFIGA 2023 A Fee is displaying in currency format as expected");
					flFIGA2023A = driver.findElement(By.xpath(loc.getProperty("buildersbindFIGA2023A"))).getAttribute("innerText").substring(1).replace(",", "").replace("\n", "");
				}
				else {
					Assert.fail("FIGA 2023A FEE IS NOT DISPLAYING IN CURRENCY FORMAT");
				}
			}
			else {
				Boolean x = true;
				try {
					driver.findElement(By.xpath(loc.getProperty("buildersbindFIGA2023A"))).getAttribute("innerText");
				} catch (Exception e) {
					x = false;
			...
			total = Double.parseDouble(totalcovprem) + Double.parseDouble(flfirefee[0]) + Double.parseDouble(flfirefee[1]) + Double.parseDouble(flFIGA2023A); //+ Double.parseDouble(flemergency)
		}
		else {
	...
	@Test(description = "T18819	TC04 - Builder Risk - Bind Submit Application - Payment Information", enabled = true, priority = 3)
	...
	public void buildersRiskBindSubmit5() throws InterruptedException {
		...
		driver.findElement(By.xpath(loc.getProperty("builderscanagentbindyes"))).click();
		driver.findElement(By.xpath(loc.getProperty("builderscanagentbindno"))).click();
	...
	public void buildersRiskBindSubmit8() throws InterruptedException {
		...
		System.out.print("\nPolicy Number: " + driver.findElement(By.xpath(loc.getProperty("builderspolicynumber"))).getText() + "\n");
	}
}
