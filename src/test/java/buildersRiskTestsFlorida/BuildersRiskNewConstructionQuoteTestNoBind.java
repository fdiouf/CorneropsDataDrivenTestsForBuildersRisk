package buildersRiskTestsFlorida;														// THIS TEST RUNS FOR 9 MINUTES AND 18 SECONDS
...
public class BuildersRiskNewConstructionQuoteTestNoBind extends BuildersRiskBaseClass {
	...
	public String[][] getData() throws IOException {
		...
		int totalrows = xlutil.getRowCount("FLNewConstructionPolicyDataNB");
		int totalcols = xlutil.getCellCount("FLNewConstructionPolicyDataNB", 1);

		String loginData[][] = new String[totalrows][totalcols];

		for (int i = 1; i <= totalrows; i++) {
			for (int j = 0; j < totalcols; j++) {
				loginData[i - 1][j] = xlutil.getCellData("FLNewConstructionPolicyDataNB", i, j);
			}
		}
		return loginData;
	}
